...
```{r}
data <- read_dta("input/data/original/pilot/FGZ Pilotstudie 2020 Betaversion.dta")
...
```{r eval=FALSE}
rp <- data_cohes_pilot %>% dplyr::select(-pid) %>%  responsePatterns::rp.acors(., max.lag = 5, na.rm = TRUE)
rp.hist(rp)
```
...
```{r}
save(data_cohes_pilot, file="input/data/proc/data_cohes_pilot.Rdata")
```
